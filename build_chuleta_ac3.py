"""Genera chuleta_AC3_oop.ipynb y chuleta_AC3_oop.py a partir de la lista CELLS.

Uso:  python3 build_chuleta_ac3.py          (crea/actualiza los dos archivos)
      python3 build_chuleta_ac3.py --run    (además ejecuta todas las celdas de código en orden)
"""
import json
import sys
import io
...
def build_py():
    out = ["# chuleta_AC3_oop.py - exportación de chuleta_AC3_oop.ipynb (mismas celdas, en orden)\n"]
    for tipo, src in CELLS:
        if tipo == "markdown":
            out.append("\n# %% [markdown]\n")
            out.extend("# " + l if l.strip() else "#" for l in src.splitlines(keepends=True))
            out.append("\n")
        else:
            out.append("\n# %% ---------------------------------------------------------\n")
            out.append(src + "\n")
...
if __name__ == "__main__":
    with open("chuleta_AC3_oop.ipynb", "w", encoding="utf-8") as f:
        json.dump(build_ipynb(), f, ensure_ascii=False, indent=1)
    with open("chuleta_AC3_oop.py", "w", encoding="utf-8") as f:
        f.write(build_py())
    print(f"{len(CELLS)} celdas -> chuleta_AC3_oop.ipynb / chuleta_AC3_oop.py")
    if "--run" in sys.argv:
        run_all()
